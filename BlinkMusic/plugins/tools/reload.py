...
from BlinkMusic.utils.database import get_authuser_names, get_cmode
from BlinkMusic.utils.decorators import ActualAdminCB, AdminActual, language
from BlinkMusic.utils.formatters import alpha_to_int
...
async def reload_admin_cache(client, message: Message, _):
    try:
        ...
        await message.reply_text(
            "Admin listesi alamadım ki. Ben yönetici değilim."
        )
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Blink.stop_stream(message.chat.id)
    except:
        pass

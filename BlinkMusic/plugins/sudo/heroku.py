...
from BlinkMusic.misc import HAPP, SUDOERS, XCB
from BlinkMusic.utils.database import (
    get_active_chats,
    remove_active_chat,
    remove_active_video_chat,
)
from BlinkMusic.utils.decorators.language import language
...
async def restart_(_, message):
    response = await message.reply_text("Yeniden başlatma..")
    served_chats = await get_active_chats()
    for x in served_chats:
        try:
            await app.send_message(
                x,
                f"{config.MUSIC_BOT_NAME} Müzik botu @hedala tarafından yeniden başlatıldı. Rahatsızlık için özür diliyor 1 dakika sonra denemenizi rica ediyorum.",
            )
            await remove_active_chat(x)
            await remove_active_video_chat(x)
        except Exception:

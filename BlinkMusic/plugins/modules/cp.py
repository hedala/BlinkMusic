...
# Önbellekleme için önbellek nesnesini oluştur
cache = TTLCache(maxsize=100, ttl=300)  # Maksimum 100 öğeyle 5 dakika süreyle önbellekleyin

@app.on_message(filters.command("coin"))
def get_crypto_price(_, message):
    crypto_symbol = message.text.split(" ", 1)[1].lower()

    crypto_price = get_cached_price(crypto_symbol)
    if crypto_price:
        send_reply_message(message, crypto_symbol, crypto_price)
    else:
...
# Önbelleğe fiyatı ekleyen yardımcı fonksiyon

...
from BlinkMusic import YouTube, app
from BlinkMusic.core.call import Blink
from BlinkMusic.misc import SUDOERS, db
from BlinkMusic.utils.database import (
    is_active_chat,
    is_music_playing,
    is_muted,
    is_nonadmin_chat,
...
from BlinkMusic.utils.formatters import seconds_to_min
from BlinkMusic.utils.inline.play import panel_markup_1, stream_markup, telegram_markup
from BlinkMusic.utils.stream.autoclear import auto_clean
from BlinkMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Blink.pause_stream(chat_id)
        ...
        await Blink.mute_stream(chat_id)
        ...
        await Blink.unmute_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Blink.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Blink.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await Blink.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_9"])
            ...
            try:
                await Blink.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await Blink.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
